...
const { startCronJob } = require('./etl-warehouse');
startCronJob();
...
async function insertInitialDataWarehouse() {
  try {
    ...
    */
    ...
    if (existingDimAngajat.length > 0 || existingDimClient.length > 0 || existingDimFactura.length > 0 || existingDimLocatie.length > 0 || existingDimMasina.length > 0 || existingDimTimp.length > 0 || existingFCursa.length > 0){
      await addMessageToDatabase('Datele initiale au fost inserate cu succes in baza de date de tip warehouse!', "I", "Admin");
    }

    */

    await sequelizeWarehouse.query('COMMIT;');
  } catch (err) {
